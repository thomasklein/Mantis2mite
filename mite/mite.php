...
class mitePlugin extends MantisPlugin {
 ...
 */
	...
	const DB_TABLE_PSMP = "projects_services_mantis_projects";
/**
 ...
 */	
	private $a_pageResources = array(
		'mite/user_account' => array('js' => array('mite_user_account.js')),
 ...
 */	
	...
	public static $a_rsrcTypes = array(mitePlugin::API_RSRC_P,mitePlugin::API_RSRC_S);
	
	public static $a_fieldNamesMiteRsrcTypes = 
		array(mitePlugin::API_RSRC_P => 'mite_project_id',
 ...
 */	
	...
	#######
		...
		return "<!-- resources mite.plugin start -->\n".
			   $s_codeToInclude . 
			   "<!-- resources mite.plugin end -->\n";
 ...
 */	
  	...
  	##################################################	
  		if ((current_user_get_field('access_level') < 
			...
			!current_user_get_field(mitePlugin::DB_FIELD_CONNECT_VERIFIED)) {
	...
	#######	
		...
		# get MITE projects/services of the user
			$s_query = "SELECT id,name,".$a_fieldNamesMiteRsrc_id[$s_type].", mite_updated_at 
						FROM ".plugin_table(mitePlugin::DB_TABLE_PS).
		...
		session_set('plugin_mite_user_projects',$a_miteUserData[mitePlugin::API_RSRC_P]);
		session_set('plugin_mite_user_services',$a_miteUserData[mitePlugin::API_RSRC_S]);
	}//initSessionVars
